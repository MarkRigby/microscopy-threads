...
author:
- name: Mark Rigby
  affiliation: Nikon Instruments UK
...
Triggered Acquisition in NIS-Elements allows the user to acquire images where some actions during the acquisition are controlled solely by hardware (TTL or analog signals) without the need for software interference and motherboard processing that can introduce delays. 
 ...
 * Multi-excitation + Piezo Z - a combination of the above mentioned applications, where a camera triggers both the Piezo Z stage and the wavelength switcher.
...
Another advantage is that since you are using the NiDAQ timing rather than the PC timing, you have more precision over when the images are acquired.
...
### Using the Triggered Acquisition Panel
...
### Using the Optical Configurations Window
